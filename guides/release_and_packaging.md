...
```bash
# Register bundle ID and download provisioning profile
mix dala.provision

# For App Store distribution
mix dala.provision --distribution

# Diagnose xcodebuild failures
# (run after a failed build — output includes actionable hints)
```
...
```bash
# Check build logs after a failed build
cat _build/prod/logs/build.log
```
